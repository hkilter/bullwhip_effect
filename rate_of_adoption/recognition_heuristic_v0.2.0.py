...
def frequency_train(c1):
    #give text about adoption price, range
    #Christen chap 17
    #http://www.interaction-design.org/encyclopedia/disruptive_innovation.html
    c1.train(' ', 'Cable TV')
    c1.train(' ', 'Camcorder')
    c1.train('Cellular and digital wireless phones have been on a disruptive path against wireline phones for 25 years.  Initially they were large, power-hungry car phones with spotty efficacy; but gradually have improved to the point where, by some estimates, nearly one-fifth of mobile telephone users have chosen to cut the cord and do without wireline telephone service.  The viability of the wireline long distance business is now in jeopardy. ', 'Cellular')
    c1.train(' ', 'CD Player')
    c1.train(' ', 'Radio')
    #Dell, Intel, Microsoft
    c1.train('Its operating system was inadequate versus those of mainframe and minicomputer makers versus Unix and versus Apple system.  But its migration from DOS to Windows to Windows NT is taking the firm up-market, to the point that the Unix world is seriously threatened.  Microsoft, in turn, faces a threat from Linux. Intel earliest microprocessor in 1971 could only constitute the brain of a four-function calculator.  Makers of computers whose logic circuitry is microprocessor-based have disrupted firms that made mainframe and minicomputers, whose logic circuitry was printed wiring board-based. Dell direct-to customer retailing model and its fast-throughput, high asset-turns manufacturing model allowed it to come underneath Compaq, IBM and Hewlett Packard as a low-end disruptor in personal computers.  Clayton Christensen, the quintessential low-end consumer, wrote his doctoral thesis on a Dell notebook computer purchased in 1989, because it was the cheapest portable computer on the market.  Because of Dell reputation for marginal quality, students needed special permission from Harvard to use doctoral stipend money to buy a Dell rather than a computer with a more reputable brand.  Today Dell supplies most of the Harvard Business School computers.', 'PC')
    c1.train(' ', 'Hybrid Corn')
    c1.train(' ', 'Tractors')
    c1.train('This firm makes a hand-held ultrasound device that enables healthcare professionals who historically needed the assistance of highly trained technicians with expensive equipment, now to look inside the bodies of patients in their care, and thereby to provide more accurate and timely diagnoses.  The company floundered for a time attempting to implement its product as a sustaining innovation.  But as of the time this book was being written, it seemed to have caught its disruptive stride in an impressive way. ', 'Ultrasound')
    c1.train(' ', 'Dishwasher')
    c1.train(' ', 'Microwave')
    c1.train(' ', 'VCR')
    c1.train(' ', 'Average')
...
def reference_train(c1):
    # Class, coefficient of innovation, coefficient of imitation
    c1.reference_class('Cable TV',0.100, 0.060)
    c1.reference_class('Camcorder', 0.044, 0.304)
    c1.reference_class('Cellular', 0.008, 0.421)
    c1.reference_class('CD Player',0.157, 0.000)
    c1.reference_class('Radio', 0.027, 0.435)
    c1.reference_class('Hybrid Corn',0.121, 0.281)
    c1.reference_class('Tractors',0.000, 0.234)
    c1.reference_class('Ultrasound',0.000,0.534)
    c1.reference_class('Dishwasher',0.000,0.179)
    c1.reference_class('Microwave', 0.002, 0.357)
    c1.reference_class('VCR', 0.025, 0.603)
    c1.reference_class('Average', 0.003, 0.380)
